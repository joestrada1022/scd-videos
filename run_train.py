...
from models.misl_net import MISLNet
...
if __name__ == "__main__":
    ...
    if net_type == 'mobile':
        net = MobileNet(constrained_net=use_constrained_layer,
        ...
        net = EfficientNet(constrained_net=use_constrained_layer,
                           global_results_dir=global_results_dir)
        net.create_model(num_classes, cnn_height, cnn_width, model_name)
        if model_path:
            net.set_model(model_path)
    elif net_type == 'misl':
        net = MISLNet(constrained_net=use_constrained_layer,
                      num_batches=num_batches,
                      global_results_dir=global_results_dir)
