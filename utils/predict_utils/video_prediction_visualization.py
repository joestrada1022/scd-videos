...
class VideoPredictionVis:
    ...
    def __plot_global(self, df):
        ...
        x_max = len(df.index)
        num_epochs = x_max - 1  # first row is zero-row
        x_range = range(0, x_max)

        # Use max loss to determine the height of y-axis
        max_loss = df['loss'].max()
        y_lim = 1 if max_loss < 1 else max_loss
        y_step = 0.05 if max_loss < 1 else 0.2
        ...
        plt.xlim(0, x_max)
        plt.ylim(0, y_lim + y_step)
        plt.xticks(np.arange(0, x_max, 5))
        plt.yticks(np.arange(0, y_lim + y_step, y_step))
        plt.grid(True)
    ...
    def __plot_platform(self, df):
        ...
        # Accuracies
        plt.plot(range(1, x_max + 1), df['acc_platform_WA'], 'g', label=f"Acc. WA")
        plt.plot(range(1, x_max + 1), df['acc_platform_YT'], 'r', label=f"Acc. YT")
        plt.plot(range(1, x_max + 1), df['acc_platform_original'], 'b', label=f"Acc. Original")

        # Loss
        plt.plot(range(1, x_max + 1), df['loss_platform_WA'], 'lightgreen', label=f"Loss WA")
        plt.plot(range(1, x_max + 1), df['loss_platform_YT'], 'lightcoral', label=f"Loss YT")
        plt.plot(range(1, x_max + 1), df['loss_platform_original'], 'lightblue', label=f"Loss Original")
    ...
    def __plot_global_and_scenario_results_separated(self, df):
        ...
        max_loss = df['loss'].max()
        y_lim = 1 if max_loss < 1 else max_loss
        y_step = 0.1 if max_loss < 1 else 0.2
    ...
    @staticmethod
    def __plot_video_results_combined(input_dict):
        ...
        for model_name in input_dict.keys():
            df_pred = input_dict[model_name]
            plt.plot(range(1, x_max), df_pred['acc'][:num_epochs], label=f"{model_name} acc.")
            plt.plot(range(1, x_max), df_pred['loss'][:num_epochs], label=f"{model_name} loss")
    ...
    @staticmethod
    def __plot_multi_results_separate(input_dict):
        plt.figure(figsize=(10, 6))
        ...
        for model_name in input_dict.keys():
            ...
            plt.subplot(len(input_dict.keys()), 1, idx)
            plt.plot(range(1, x_max), df_pred['acc'], label=f"Accuracy")
            plt.plot(range(1, x_max), df_pred['loss'], label=f"Loss")
    ...
    def create_cm(input_file, class_names, scenario=None, platform=None):
        ...
        color_bar_lbl = 'No. videos per class'
        ...
        ax = sn.heatmap(df_cm, annot=True, square=False, cmap="YlGnBu", cbar_kws={'label': color_bar_lbl}, vmin=0,
                        vmax=cm_matrix.max())
    ...
    def create_cm_normalized(input_file, class_names, scenario=None, platform=None):
        ...
        color_bar_lbl = 'Normalized num videos per class'
        title = f"Video Confusion Matrix"
    ...
    def create_cm_normalized_thesis_ordering(input_file, class_names, scenario=None, platform=None):
        ...
        color_bar_lbl = 'Normalized Classification Accuracy'
