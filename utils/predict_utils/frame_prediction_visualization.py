...
class FramePredictionVis:
    ...
    def __plot_platform(df):
        ...
        # Platform accuracies
        plt.plot(range(1, x_max + 1), df['acc_platform_WA'], 'g', label=f"Whatsapp")
        plt.plot(range(1, x_max + 1), df['acc_platform_YT'], 'r', label=f"YouTube")
        plt.plot(range(1, x_max + 1), df['acc_platform_original'], label=f"Original")
