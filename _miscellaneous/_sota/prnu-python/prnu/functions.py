...
def noise_extract_compact(args):
    ...
    im = args[0]
    # im = inten_scale(im) * saturation(im)
...
def preprocessing_wrapper(img_path):
    im_arr = cv2.imread(img_path)
    shape = im_arr.shape
    if shape[0] > shape[1]:  # The video was captured in portrait mode (changing to landscape)
        # The source EXIF for corresponding video does not specify rotate clockwise by 90 or counter-clockwise by 90
        # We arbitrarily choose 90 degrees clockwise. (Unfortunately, not an ideal scenario)
        im_arr = cv2.rotate(im_arr, cv2.ROTATE_90_CLOCKWISE)

    assert im_arr.dtype == np.uint8, 'Error while reading image: {}'.format(img_path)
    assert im_arr.ndim == 3, 'Image is not RGB: {}'.format(img_path)
    return cv2.resize(im_arr, (480, 800))
    # return cut_ctr(im_arr, (480, 800, 3))  # Center crop
...
def stats(cc: np.ndarray, gt: np.ndarray, ) -> dict:
    ...
    fpr, tpr, th = roc_curve(gt.flatten(), cc.flatten(), drop_intermediate=False)
