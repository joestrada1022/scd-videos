#!/bin/bash
#SBATCH --job-name=mn-allIframes-ccrop
...
#SBATCH --chdir=/scratch/p288722/runtime_data/scd_videos_first_revision/06_I_frames
...
#echo --homo_or_not=${homo_or_not} --net_type=${net} --dataset=${splits_dir} --epochs=20 --lr=0.1 --batch_size=64 --height=480 --width=800 --use_pretrained=1 --gpu_id=0 --const_type=${const_type} --model_name="${model_name}" --global_results_dir="${base_dir}/${num_frames}_frames/${net}_net"
...
# Adding content-root to Python Path (to allow execution of scripts not present in the root directory)
